...
public class DiscordPlug implements NotificationPushPlug {
	...
	public void connect() {
		// try {
		// DiscordRPC.discordInitialize("742825259190714439", new
		// DiscordEventHandlers.Builder().setReadyEventHandler(user -> username
		// = user.username).build(), true);
		// } catch (Exception e) {
		// Log.warn("Exception on initializing Discord RPC : " +
		// e.getMessage());
		// }
	}
	...
	public void push(Notification notification) {
		// DiscordRichPresence.Builder rich = new
		// DiscordRichPresence.Builder(notification.getStatus()).setDetails(notification.getDetails()).setBigImage(notification.getImage(),
		// "");
		// if (notification.getParty() != null) {
		// rich.setParty("Party", notification.getParty().getActual(),
		// notification.getParty().getMaximum());
		// }
